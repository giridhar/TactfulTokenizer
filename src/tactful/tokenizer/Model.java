...
public class Model {
	...
	// We can add additional features here when we know that the feature set doens't not exist in training data
	private void addAdditionalFeatures() {
		features.put("w1_Clarence", 0.0);
		features.put("w1_?", 0.0);
	}


	public static int getLower_words(String string) {
	...
	public static int getNon_abbrs(String string) {
		Integer result = new Integer(0);
	...
	public static double getFeats(String string) throws IOException {
		...
		System.out.println("getFeats for :"+string+"="+result);
		if (result != null)
			return result;
		else
			return 1;
	...
	public static void main(String args[]) throws IOException {
		...
		// String[] re =
		// model.tokenize_text("this is u.s.a. please come home! where are you? This is meeting prep. for you.");
		// String[] re = model.tokenize_text("this is u.s.a. for you.");
		// String[] re =
		// model.tokenize_text("Rs. 109 Limited Time Domain Sale! Book Your Domain And Get Online Today.");
		// String[] re =
		// model.tokenize_text("Rs. 109\n\n  Limited Time \nDomain Sale! Book Your Domain    And \nGet Online Today.");
		String[] re = model
				.tokenize_text("N.Y. is the biggest city of U.S.A. and Auckland is the biggest city of N.Z.");
		/*
		 * Scanner sc=new Scanner(System.in);
		 * System.out.println("Please enter the Line to pass"); String input =
		 * sc.nextLine(); String[] re = model.tokenize_text(input);
		 */
//		for (String print : re) {
...
//		}
	...
	// this method will set score pred(probability of having sentance) from
	// features model.
	private void classify(Doc data) throws IOException {
		...
		for (Frag frag : data.frags) {
			...
			for (String feat : frag.features) {
				probs *= getFeats(feat);
				//System.out.println("passing "+feat+" coming "+getFeats(feat)+" times "+i++);
			}
	...
	public void get_features(Frag frag, Model model) {
		...
		if (w2 != null && !w2.isEmpty()) {
			if ((w1.substring(0, w1.length() - 1).matches("[a-zA-Z]+"))) {
				...
				frag.features.add("w1abbr_"
						+ Model.getNon_abbrs(w1.substring(0, w1.length() - 1)));
			}

			if (!(w2.substring(0, w2.length() - 1).matches(".*\\d+"))) {
				frag.features.add("w2cap_"
						+ !Character.isUpperCase(w2.charAt(0)));
				frag.features.add("w2lower_"
						+ Model.getLower_words(w2.toLowerCase()));
			}
		}
	}
}
